...
# Run for each Peer connected
def handle_peer_connection(peer_handler_socket, peer_addr):
    ...
    peer_handler_socket.send(bencode(f"Tracker established connection to Peer[{
                             peer_ip},{peer_port}]").encode(FORMAT))  # send to peer
        ...
        if received_msg:
            ...
            match msg_parts[0]:
                case "/get_peer_set":
                    ...
                    peer_handler_socket.send(bencode(PEER_SET).encode(
                        FORMAT))  # send PEER SET string
                    connected = True
                case "/update_status_to_tracker":
                    ...
                    peer_handler_socket.send(
                        bencode(f"Tracker updated Peer[{peer_ip},{peer_port}] status").encode(FORMAT))
                case "/disconnect_tracker":
                    print(f"[PEER DISCONNECTED TRACKER] {peer_ip},{peer_port}")
                    peer_handler_socket.send(bencode(f"Peer[{peer_ip},{
                                             peer_port}] disconnected from tracker").encode(FORMAT))  # send to peer
                    connected = False
                case "/quit_torrent":
                    print(f"[PEER QUITED TORRENT] {peer_ip},{peer_port}")
                    peer_handler_socket.send(bencode(
                        # send to peer
                        f"Peer[{peer_ip},{peer_port}] quited torrent").encode(FORMAT))
...
def start():
    ...
    while True:
        # detect a client connect
        peer_handler_socket, peer_addr = tracker_listening_socket.accept()
        thread = threading.Thread(target=handle_peer_connection, args=(
            peer_handler_socket, peer_addr))  # create a "listening peer" thread
        thread.start()
